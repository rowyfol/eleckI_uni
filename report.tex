...
\node[nand gate, draw=accent, line width=1.5pt, logic gate inputs=nn, minimum width=1.4cm, minimum height=1cm] (nand1) at (4.5,2.2) {};
\node[nand gate, draw=accent, line width=1.5pt, logic gate inputs=nn, minimum width=1.4cm, minimum height=1cm] (nand2) at (4.5,-2.2) {};
\node[nand gate, draw=secondary, line width=1.5pt, logic gate inputs=nn, minimum width=1.4cm, minimum height=1cm] (nand3) at (9,0) {};

% --- Inputs ---
\draw[primary, line width=1.2pt, -{Stealth}] (-2.5,0) -- (not.input) 
  node[pos=0.35, above, font=\small, fill=white, inner sep=2pt] {$S$};

\draw[primary, line width=1.2pt, -{Stealth}] (-2.5,2.2) -- (nand1.input 1) 
  node[pos=0.35, above, font=\small, fill=white, inner sep=2pt] {$D_0$};
\draw[primary, line width=1.2pt, -{Stealth}] (-2.5,-2.2) -- (nand2.input 2) 
  node[pos=0.35, below, font=\small, fill=white, inner sep=2pt] {$D_1$};

% --- S branch to NAND2 (top input) ---
\draw[primary, line width=1pt] (-2.5,0) -- (-1.2,0);
\fill[primary] (-1.2,0) circle (1.5pt);  % junction dot
\draw[primary, line width=1pt] (-1.2,0) -- (-1.2,-1.6) -- (nand2.input 1);

% --- Sbar from NOT to NAND1 (bottom input) ---
\draw[primary, line width=1pt] (not.output) -- (1.5,0) -- (1.5,1.6) -- (nand1.input 2);
\node[font=\small, fill=white, inner sep=2pt, anchor=west] at (1.55,0.8) {$\overline{S}$};

% --- NAND1 / NAND2 outputs to NAND3 ---
...
% --- Final output ---
\draw[secondary, line width=1.5pt, -{Stealth}] (nand3.output) -- ++(1.5,0) 
  node[right, font=\small, fill=white, inner sep=2pt] {$Y$};

% --- Gate labels ---
\node[font=\footnotesize\sffamily\bfseries, color=accent, above=0.25cm of nand1] {NAND1};
\node[font=\footnotesize\sffamily\bfseries, color=accent, below=0.25cm of nand2] {NAND2};
\node[font=\footnotesize\sffamily\bfseries, color=secondary, above=0.25cm of nand3] {NAND3};
...
\subsection{Base Resistor (10K$\Omega$)}

The base resistor controls how much current enters the transistor base. For the transistor to act as a solid switch, it must be in \textit{deep saturation}---not just barely on.

The collector current is set by the pull-up resistor:
\begin{equation}
I_C \approx \frac{V_{CC}}{R_C} = \frac{5\,\text{V}}{1\,\text{k}\Omega} = 5\,\text{mA}
\end{equation}

To guarantee saturation, we use a forced beta ($\beta_{forced}$) much lower than the transistor's actual current gain (typically 100--300 for 2N3904). We chose $\beta_{forced} \approx 12$:

\begin{equation}
I_B = \frac{I_C}{\beta_{forced}} = \frac{5\,\text{mA}}{12} \approx 0.4\,\text{mA}
\end{equation}

Then the base resistor is:
\begin{equation}
R_B = \frac{V_{CC} - V_{BE}}{I_B} = \frac{5\,\text{V} - 0.7\,\text{V}}{0.4\,\text{mA}} \approx 10.7\,\text{k}\Omega
\end{equation}

We used the standard value of 10K$\Omega$.
\subsection{Collector Pull-Up Resistor (1K$\Omega$)}

The pull-up resistor determines the logic HIGH level and the current drawn when the transistor is ON.

\begin{itemize}[leftmargin=2em]
  \item \textbf{Logic HIGH:} When the transistor is OFF, the pull-up holds the output near 5V.
  \item \textbf{Power:} When the transistor is ON, current flows through the resistor to ground. With $R_C = 1\,\text{k}\Omega$, this current is about 5mA---enough to drive the next stage but not so much that the transistor overheats.
  \item \textbf{Speed:} A smaller resistor charges the parasitic capacitance faster, but draws more power. 1K$\Omega$ is a standard compromise for RTL.
\end{itemize}

\subsection{LED Current-Limiting Resistor (330$\Omega$)}

The LED needs a specific current to glow brightly without burning out. Red LEDs typically drop about 1.8V.

\begin{equation}
I_{LED} = \frac{V_{CC} - V_{LED} - V_{CE(sat)}}{R_{LED}} = \frac{5\,\text{V} - 1.8\,\text{V} - 0.2\,\text{V}}{330\,\Omega} \approx 9\,\text{mA}
\end{equation}

This is a safe, bright current for a standard 5mm LED.

\subsection{Power Supply (LM7805)}
...
\caption{The MUX constructed on a solderless breadboard. Power is supplied by an LM7805 5V voltage regulator.}
...
\begin{tikzpicture}
  \node[draw=primary, line width=1.5pt, rounded corners=5pt, inner sep=15pt, fill=lightgray] (box) at (0,0) {
    ...
    {\small\sffamily\textcolor{primary}{\textbf{\url{https://github.com/rowyfol/Elec101}}}}
    \end{minipage}
  };
\end{tikzpicture}
\end{center}
