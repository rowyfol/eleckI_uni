...
\begin{keypoint}
\textbf{Why this matters:} If $R_B$ is too large, the transistor barely turns on and stays in the active region. The output voltage floats somewhere between 0V and 5V instead of snapping to a clean logic level.
\end{keypoint}
...
\begin{keypoint}
\textbf{Why a regulator matters:} Without the LM7805, a fresh 9V battery would start at about 9.6V and drop over time. The changing supply would shift the logic HIGH level and alter the base current, making the circuit behave inconsistently. The LM7805 fixes the rail at 5V, so the transistor switching stays predictable.
\end{keypoint}
